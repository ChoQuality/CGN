class AWPMessengerDorothyViewEvent {
    constructor() {
        this._isPaging = true;
        ...
        this._createElementEvent = (createElementEvent,element,roomAWPMessageDetailListMap,mvc,clsMessage,companyOrgUser,contentViewElement) => {
            this._isPaging = true;
            if(createElementEvent === false){
                ...
                roomAWPMessageDetailList.forEach(roomAWPMessageDetail => {
                    ...
                    const htmlElementFunc = (htmlElement) => {
                        return mvc.DorothyChatBox(roomAWPMessageDetail, htmlElement,companyOrgUser);
                    };
                    const htmlElement = mvc.DorothyChatCardGroup(roomAWPMessageDetail, true);
                    if (first) {
                        chatMessageBody.prepend(mvc.chatBoxDivider(roomAWPMessageDetail));
                    }
                    chatMessageBody.prepend(htmlElementFunc(htmlElement));
                });
                const chatBox__chatList = chatMessageBody.querySelectorAll('.chatBox__chat');

                if (chatBox__chatList.length > 0) {
                    const lastChat = chatBox__chatList[chatBox__chatList.length - 1];
                    if(!lastChat.classList.contains('is-dorothy')){
        ...
        this._createSendEvent = (createSendEvent,element,func,clsMessage,formData,contentViewElement) => {
            if(createSendEvent === false){
                ...
                sendMessageArea.removeEventListener('keyup', handleKeyUp);
                ...
                function handleSendClick() {
                    ...
                    messageText !== '' ? contextMap.set(contextType.TEXT,messageContentVal) : contextMap.set(contextType.TEXT,null);
                    clsMessage.publishRoomMessage(currentRoomList,messageType.TALK,contextMap);
                    clsMessage.isDorothyProcessEventHandler(true);
                }

                sendMessageArea.addEventListener('keypress', handleKeyUp);
        ...
        this._createSearchEvent = (createSearchEvent,element,func,mvc,loginInfo,companyOrgUser,clsMessage,innerEvent) => {
            if(createSearchEvent === false){
                ...
                const searchBtnEventHandler = () => {
                    createSearchEvent = func.dorothySearchBtnEventHandler(createSearchEvent,msgSearch, msgSearchForm, msgSearchList, element, mvc, loginInfo,companyOrgUser,clsMessage,innerEvent,'dorothy');
                };
                        ...
                        fetch(msgPagingUrl, {
                            ...
                            .then(response => {
                                ...
                                tempList
                                    ...
                                    .forEach( (item) => {
                                        const awpMessageDetail = new AWPMessageDetail(loginInfo,Number(item.messageId),func.makeMessage(item),func.makeUpperMessageApi(item.upperMessageDto),previousMessageDetail);
                            ...
                            .then((roomAWPMessageDetailList) => {
                                if(roomAWPMessageDetailList.length !== 0){
                                    ...
                                    this.createElementEvent(false,element,clsMessage.roomAwpMessageDetailListMap,mvc,clsMessage,companyOrgUser,contentViewElement);
